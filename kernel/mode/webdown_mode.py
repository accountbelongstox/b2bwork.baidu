...
class WebdownMode(BaseClass):
    ...
    def down_website_url_to_file_format(self, current_url, tag_type, ele, base_url):
        ...
        # link文件命名规则
            ...
            if (url_offset_link := self.down_website_source_link_suffix(ele, url_offset, url_path, url_query,
                                                                        "rel")) != None:
                url_offset = url_offset_link
            elif (url_offset_link := self.down_website_source_link_suffix(ele, url_offset, url_path, url_query,
                                                                          "type")) != None:
                url_offset = url_offset_link
        # a连接文件命名规则
    ...
    def down_website_foreach_tag(self):
        ...
        while True:
            ...
            if self.__down_web_driver != None and tab_threads_ok > 0:
                ...
                WebDriverWait(self.__down_web_driver, timeout=1).until(
                    EC.visibility_of(driver.find_element(By.TAG_NAME, "html")))
                try:
